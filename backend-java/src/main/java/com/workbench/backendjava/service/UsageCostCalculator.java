...
/**
 * Admin 用量页：按 {@code application.yml} 定价表将 token / 张数估算为人民币区间。
 */
@Component
...
public class UsageCostCalculator {
    ...
    /**
     * 估算单次调用的费用区间（支持 DeepSeek 输入/输出分档与按张计费）。
     *
     * @param scene 业务场景（如 matting_detect 可推断默认 VL 模型）
     */
    ...
    /**
     * 按 model/provider/scene 解析定价表 key 后取官方文档链接。
     * 调用方：{@link AdminUserService} 用量明细。
     */
    ...
    /** 直接按 pricingKey 取配置中的 sourceUrl。 */
    ...
    /** 将 scene 缺省模型（如 matting_detect → qwen-vl-max）解析为有效 model/provider。 */
    ...
    /** model / provider / alias 映射到 {@code ai.pricing.models} 的 key。 */
